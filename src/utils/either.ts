import { Nullable, Thunk, Unary } from "@utils/types";
...
export interface IEitherStatic {
  ...
  try: <TRight>(thunk: Thunk<TRight>) => IEither<TRight, Error>;
  fromNullable: <TRight>(
    x: Nullable<TRight>
  ) => IEither<NonNullable<TRight>, null>;
  fromBoolean: <TRight, TLeft = undefined>(
    check: Unary<TRight, boolean>,
    value: TRight,
    onFalse?: TLeft
...
export const Either: IEitherStatic = {
  ...
  fromBoolean: (check, value, onFalse = undefined) =>
    check(value) ? Either.right(value) : Either.left(onFalse as any),
